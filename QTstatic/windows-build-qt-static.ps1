...
# Change the $QtSrcUrl to the version you prefer to compile (MUST BE AGREE WITH INSTALLED Dynamic pre-built Qt)
[CmdletBinding()]
param(
...
function Main
{
    ...
    if (-not $MingwDir) {
        ...
        # Incorrect gcc.exe location in the code (SET UP correct one for your location)
        # The original was: $GccList = @(Get-ChildItem -Path C:\Qt\*\Tools\mingw*\bin\gcc.exe | ForEach-Object FullName | Sort-Object)
        $GccList = @(Get-ChildItem -Path C:\Qt\Tools\mingw482_32\bin\gcc.exe | ForEach-Object FullName | Sort-Object)
...
function Get-7zip
{
    # Change the PATH to your 7z program
    # The original was: $Exe = "C:\Program Files\7-Zip\7z.exe"
    $Exe = "C:\bin\7za.exe"
